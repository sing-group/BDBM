...
public class DefaultProCompartBinariesChecker implements ProCompartBinariesChecker {
	private ProCompartBinaries eBinaries;
	
	public DefaultProCompartBinariesChecker() {
	}
	
	public DefaultProCompartBinariesChecker(ProCompartBinaries eBinaries) {
		this.eBinaries = eBinaries;
	}
	
	public static void checkAll(ProCompartBinaries eBinaries)
	throws BinaryCheckException {
		new DefaultProCompartBinariesChecker(eBinaries).checkAll();
	}

	@Override
	public void setBinaries(ProCompartBinaries eBinaries) {
		this.eBinaries = eBinaries;
	}

	@Override
	public void checkAll() throws BinaryCheckException {
		this.checkProCompart();
	}

	@Override
	public void checkProCompart() throws BinaryCheckException {
		checkCommand(this.eBinaries.getProCompart());
	...
	protected static void checkCommand(String command) throws BinaryCheckException {
		...
		try {
			...
			if (!sb.toString().startsWith("This script requires two arguments")) {
				throw new BinaryCheckException("Unrecognized version output", command);
			}
